...
export default function AdminWorkspaceScreen() {
  ...
  const removeTaxonomy = (item: Taxonomy) => Alert.alert('Xóa taxonomy', `Xóa ${item.name}?`, [{ text: 'Hủy', style: 'cancel' }, { text: 'Xóa', style: 'destructive', onPress: () => mutate(async () => { await adminApi.removeTaxonomy(item._id); setTaxonomies((items) => items.filter((value) => value._id !== item._id)); }) }]);
  ...
  const renderTab = () => {
    ...
    if (tab === 'taxonomies') return <><Card style={styles.card}><Text style={styles.heading}>{editingTaxonomy ? 'Cập nhật taxonomy' : 'Thêm taxonomy'}</Text>{!editingTaxonomy ? <ChipSelect label="Loại" options={[{ value: 'skill', label: 'Skill' }, { value: 'major', label: 'Major' }, { value: 'category', label: 'Category' }]} value={taxonomyForm.type} onChange={(type) => setTaxonomyForm({ ...taxonomyForm, type })} /> : null}<TextField label="Tên" value={taxonomyForm.name} onChangeText={(name) => setTaxonomyForm({ ...taxonomyForm, name })} /><TextField label="Mô tả" value={taxonomyForm.description} onChangeText={(description) => setTaxonomyForm({ ...taxonomyForm, description })} /><View style={styles.actions}>{editingTaxonomy ? <Button label="Hủy" variant="secondary" onPress={() => { setEditingTaxonomy(null); setTaxonomyForm({ type: 'skill', name: '', description: '' }); }} /> : null}<Button label={editingTaxonomy ? 'Lưu' : 'Thêm'} loading={saving} onPress={saveTaxonomy} /></View></Card><Card style={styles.card}><Text style={styles.heading}>Danh mục hiện có</Text>{taxonomies.map((item) => <View key={item._id} style={styles.row}><View style={styles.flex}><Text style={styles.item}>{item.name}</Text><Text style={styles.muted}>{item.type} · {item.isActive === false ? 'Tắt' : 'Bật'}</Text></View><Button label="Sửa" size="sm" onPress={() => { setEditingTaxonomy(item._id); setTaxonomyForm({ type: item.type, name: item.name, description: item.description || '' }); }} /><Button label="Xóa" size="sm" variant="danger" onPress={() => removeTaxonomy(item)} /></View>)}</Card></>;
    if (tab === 'notifications') return <Card style={styles.card}><Text style={styles.heading}>Gửi notification</Text><ChipSelect label="Đối tượng" options={[{ value: 'audience', label: 'Theo nhóm' }, { value: 'recipient', label: 'Theo user ID' }]} value={notificationForm.mode} onChange={(mode) => setNotificationForm({ ...notificationForm, mode })} />{notificationForm.mode === 'recipient' ? <TextField label="Recipient user ID" value={notificationForm.recipient} onChangeText={(recipient) => setNotificationForm({ ...notificationForm, recipient })} /> : <ChipSelect label="Audience" options={[{ value: 'all', label: 'Tất cả' }, { value: 'student', label: 'Sinh viên' }, { value: 'lecturer', label: 'Giảng viên' }, { value: 'admin', label: 'Quản trị viên' }, { value: 'club_leader', label: 'Chủ nhiệm CLB' }]} value={notificationForm.audience} onChange={(audience) => setNotificationForm({ ...notificationForm, audience })} />}<TextField label="Nội dung" multiline value={notificationForm.message} onChangeText={(message) => setNotificationForm({ ...notificationForm, message })} /><TextField label="Link (tùy chọn)" value={notificationForm.link} onChangeText={(link) => setNotificationForm({ ...notificationForm, link })} autoCapitalize="none" /><Button label="Gửi notification" loading={saving} onPress={sendNotification} />{recipientCount !== null ? <Text style={styles.success}>Đã gửi đến {recipientCount} người nhận.</Text> : null}</Card>;
  ...
  if (stats.loading || activity.loading) return <ScreenShell title="Admin workspace" onBack={() => router.back()}><LoadingState /></ScreenShell>;
  if (stats.error) return <ScreenShell title="Admin workspace" onBack={() => router.back()}><ErrorState message={stats.error} onRetry={stats.reload} /></ScreenShell>;
  const tabs: [Tab, string][] = [['overview', 'Tổng quan'], ['clubs', 'Clubs'], ['registrations', 'Đơn CLB'], ['taxonomies', 'Taxonomies'], ['notifications', 'Thông báo'], ['reports', 'Reports'], ['settings', 'Settings']];
  return <ScreenShell title="Admin workspace" onBack={() => router.back()}><ScrollView contentContainerStyle={styles.body} horizontal={false}><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.tabs}>{tabs.map(([key, label]) => <Pressable key={key} style={[styles.tab, tab === key && styles.activeTab]} onPress={() => setTab(key)}><Text style={tab === key ? styles.activeText : styles.tabText}>{label}</Text></Pressable>)}</ScrollView>{renderTab()}</ScrollView></ScreenShell>;
}
